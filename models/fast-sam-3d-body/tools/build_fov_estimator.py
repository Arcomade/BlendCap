...
def load_moge(device, path="", half=True, use_trt=False):
    ...
    # Offline-first: if BlendCap has bundled this MoGe model locally, load the
    # checkpoint straight off disk instead of reaching Hugging Face. save_mhr_
    # data.py sets BLENDCAP_MOGE_BASE to the checkpoints dir; the per-model
    # subfolder is the repo's short name (e.g. "moge-2-vits-normal") and the
    # installer pre-fetches model.pt there. MoGeModel.from_pretrained() loads a
    # local file path directly (no network) when the path exists.
    # [BlendCap offline patch — keep when re-vendoring the Fast lib.]
    _base = os.environ.get("BLENDCAP_MOGE_BASE", "")
    if _base and isinstance(path, str) and "/" in path:
        _local_pt = os.path.join(_base, path.split("/")[-1], "model.pt")
        if os.path.isfile(_local_pt):
            print(f"  [MoGe] Using local checkpoint (offline): {_local_pt}")
            path = _local_pt
