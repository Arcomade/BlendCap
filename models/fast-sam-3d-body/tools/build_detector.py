...
def run_yolo11(
    ...
    # Check if using TensorRT engine or ONNX (neither needs explicit half — engine
    # has it baked in, ONNX runs via ORT which does its own dtype handling).
    # Matches load_yolo11's is_engine check (line 138).
    model_name = str(getattr(detector, 'model_name', getattr(detector, 'ckpt_path', '')))
    is_engine = model_name.endswith('.engine') or model_name.endswith('.onnx')

    # ultralytics half=True calls .half() on tensors, which requires CUDA.
    # On non-CUDA devices (CPU, DirectML, MPS), requesting half causes
    # "Torch not compiled with CUDA enabled" inside ultralytics.
    # Handle both string ("cuda") and torch.device objects.
    on_cuda = 'cuda' in str(device).lower()
    ...
    results = detector(
        ...
        imgsz=imgsz,
        device=device,
        half=(not is_engine) and on_cuda,  # FP16 only on CUDA + PyTorch models
    )
