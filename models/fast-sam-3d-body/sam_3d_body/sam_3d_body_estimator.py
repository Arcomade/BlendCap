...
class SAM3DBodyEstimator:
    ...
    def process_one_image(
        ...
        if cam_int is not None:
            ...
            torch.cuda.synchronize()
            batch["cam_int"] = cam_int.clone()
            # [BlendCap FOV-cache patch - keep when re-vendoring the Fast lib]
            # Expose the MoGe-estimated intrinsics so the capture loop can cache
            # them and skip MoGe on later frames (focal length is constant within
            # a clip). Read by save_mhr_data.py's FOV cache (getattr last_cam_int).
            self.last_cam_int = cam_int.detach().clone()
        else:
