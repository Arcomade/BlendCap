...
class Dinov3Backbone(nn.Module):
    def __init__(
        ...
        # Load the DINOv3 repo from a LOCAL copy so capture works fully
        # offline. This loads only the model architecture CODE (not weights —
        # the weights come from the SAM 3D Body checkpoint, hence
        # pretrained=False). BlendCap ships the repo inside the addon and
        # points DINOV3_HUB_DIR at it; we also honour torch's hub dir (which
        # follows TORCH_HOME) and the legacy ~/.cache location. source="local"
        # NEVER contacts GitHub, so there is no network round-trip — no offline
        # crash and no slow-start stall while a doomed github.com probe times
        # out. Only if NO local copy is found do we fall back to online.
        # [BlendCap offline patch — keep when re-vendoring the Fast lib.]
        _repo_name = "facebookresearch_dinov3_main"
        _candidates = []
        _env_dir = os.environ.get("DINOV3_HUB_DIR", "")
        if _env_dir:
            _candidates.append(_env_dir)
        try:
            _candidates.append(os.path.join(torch.hub.get_dir(), _repo_name))
        except Exception:
            pass
        _candidates.append(os.path.expanduser(f"~/.cache/torch/hub/{_repo_name}"))
        local_cache = next(
            (p for p in _candidates if p and os.path.isdir(p)), None
        )

        if local_cache is not None:
            # Offline path: build the architecture from the local repo.
            ...
            # No local copy found — last-resort online fetch (needs internet).
            self.encoder = torch.hub.load(
