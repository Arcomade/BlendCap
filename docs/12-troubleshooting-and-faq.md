...
For poses on the floor (lying, rolling, crawling, kneeling), **Body Grounding** in BVH Post Processing does the seating and is on by default, so first check it's still enabled and that **Catch Distance** is large enough to reach your pose. For standing motion, use **Foot Locking**: for sliding, and for feet that float during holds and deep squats, raise **Sensitivity**; for brief pops, turn on **Bridge Phantom Lifts**. For a whole performance floating above the floor, or bobbing and drifting vertically (a moving camera does this), try **Feet Define the Floor** in Foot Locking Settings; if the level is still off, you can nudge **Height** in Camera Angle Offset, then **Bake Settings to BVH** to make it permanent. If the entire body is sliding around, you may need to turn off root motion and animation movement along the ground plane manually. See [BVH Generation and Post-Processing](08-bvh-generation-and-post-processing.md). You may also want to make sure your capture had the FOV set to a constant value, or manually enter the camera's focal length. See [Capturing](05-capturing.md).
